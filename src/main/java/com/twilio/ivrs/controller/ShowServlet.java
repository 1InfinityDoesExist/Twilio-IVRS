...
import com.twilio.ivrs.utils.WebAppServlet;
...
public class ShowServlet extends WebAppServlet {
	...
	private VoiceResponse getReturnInstructions() {
		Say firstPhrase = new Say.Builder("To get to your extraction point, get on your bike and go down "
		...
		VoiceResponse voiceResponse = new VoiceResponse.Builder().say(firstPhrase).say(secondPhrase).hangup(hangup)
				.build();
	...
	private VoiceResponse getPlanets() {
		Say phrase = new Say.Builder("To call the planet Broh doe As O G, press 2. To call the planet "
				...
				+ "go back to the main menu, press the star key ").voice(Say.Voice.ALICE).language(Say.Language.EN_GB)
						.loop(3).build();

		Gather gather = new Gather.Builder().say(phrase).action("/extensions/connect").numDigits(1).build();
